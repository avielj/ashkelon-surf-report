...
The Home Assistant integration now lives under `custom_components/ashkelon_surf` so it can be installed directly through HACS. The documentation moved with it and covers HACS setup, manual installation, attributes, automations, and troubleshooting.

👉 Open `custom_components/ashkelon_surf/README.md` for the up-to-date guide.

This legacy folder is kept for the update scripts (`update_from_github.py` / `.sh`).
cards:
  - type: markdown
    content: |
